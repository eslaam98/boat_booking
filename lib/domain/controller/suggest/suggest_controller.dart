...
abstract class SuggestController extends GetxController {
  addSuggest();
}

class SuggestControllerImp extends SuggestController {
  AddSuggestData addSuggestData = AddSuggestData(Get.find());
  ...
  addSuggest() async {
    if (formstate.currentState!.validate()) {
      ...
      update();
      var response = await addSuggestData.postdata(
          sharedService.sharedPref.getString("user_id")!,
          suggestUsername.text,
          suggestUserPhone.text,
          suggestType.text,
          suggestDesc.text);
      if (kDebugMode) {
